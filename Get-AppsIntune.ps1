...
function Get-AuthToken {
    ...
    if($Devices){
        ...
        foreach($DetectedApp in $DetectedApps){
            ...
            $MergeObject = New-Object -TypeName PSObject
            $MergeObject | Add-Member -MemberType NoteProperty -Name DeviceName -Value $Device.deviceName
            $MergeObject | Add-Member -MemberType NoteProperty -Name displayName -Value $DetectedApp.displayName
            $MergeObject | Add-Member -MemberType NoteProperty -Name Version -Value $DetectedApp.version
            $mergeCollection += $MergeObject
    ...
    write-host "Não foram encontrados nenhum dispositivo... :(" -f green
    Write-Host
